...
module.exports.run = async function({ api, event, Users, Threads }) {
  ...
  const { PREFIX } = global.config;
    ...
    if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
        console.log(1)
        return api.sendMessage("✨🍀Mirai Team✨🍀\n[🍀] Kết nối thành công\n [🌸] Dấu lệnh / \n [⌛] Admin :https://www.facebook.com/dead.cellss", threadID, async () => {
            let check = true;
            while (check) {
        ...
        try {
            ...
            memLength.sort((a, b) => a - b);
          	...
          	(typeof threadData.customJoin == "undefined") ? msg = "[🍀Mirai Bot🍀] =>Chào Mừng {name}!\n[🍀Mirai Bot🍀]Chào mừng đã đến với {threadName}!\n[🍀Mirai Bot🍀]{type} là thành viên thứ {soThanhVien} của nhóm❤️\n[🍀Mirai Bot🍀]Chúc {type} có một {time} vui vẻ\n[🍀Mirai Bot🍀]Ngày vào: {fullYear}"  : msg = threadData.customJoin;
            msg = msg
